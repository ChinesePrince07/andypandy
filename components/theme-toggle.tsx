...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      {/* Sun icon */}
      <svg
        className={`h-4 w-4 transition-all ${dark ? "scale-0 rotate-90 opacity-0 absolute" : "scale-100 rotate-0 opacity-100"}`}
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth={2}
        stroke="currentColor"
      ...
      {/* Moon icon */}
      <svg
        className={`h-4 w-4 transition-all ${dark ? "scale-100 rotate-0 opacity-100" : "scale-0 -rotate-90 opacity-0 absolute"}`}
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth={2}
        stroke="currentColor"
