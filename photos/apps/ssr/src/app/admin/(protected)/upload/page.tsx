...
export default function UploadPage() {
  ...
  const toggleFileWorkout = useCallback((index: number) => {
  ...
  const toggleAllWorkout = useCallback(() => {
    setFiles((prev) => {
      ...
      return prev.map((f) => (f.status === 'pending' ? { ...f, isWorkout: anyPublicPending } : f))
    })
  }, [])
        ...
        <div className="mt-8">
          ...
          {!isUploading && pendingCount > 0 && (
            <div className="mb-5 flex items-center gap-2">
              ...
              >
                {files.some((f) => f.status === 'pending' && !f.isWorkout) ? 'Mark All Workout' : 'Unmark Workout'}
              </button>
            </div>
          )}
            ...
            {files.map((uploadFile, index) => (
              ...
              >
                ...
                <div className="px-3 py-2.5">
                  ...
                  {uploadFile.status === 'pending' && (
                    <div className="mt-2">
                      ...
                      <label className="mt-2 flex cursor-pointer items-center gap-1.5 text-[11px] text-neutral-400">
                        <input
                          type="checkbox"
                          checked={uploadFile.isWorkout}
                          onChange={() => toggleFileWorkout(index)}
                  ...
                  {uploadFile.status !== 'pending' && (uploadFile.tags.length > 0 || uploadFile.isWorkout) && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {uploadFile.isWorkout && (
                        <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[10px] text-amber-400">workout</span>
                      )}
                      {uploadFile.tags.map((tag) => (
